...
// SkipBillOnEmptyResult 判定当前请求所属用户是否在「空结果不计费」白名单内。
// 名单由 quota_setting.skip_bill_on_empty_result_user_ids 维护，仅管理员可改。
func SkipBillOnEmptyResult(info *relaycommon.RelayInfo) bool {
	return info != nil && operation_setting.IsSkipBillOnEmptyResultUser(info.UserId)
}

// applyEmptyResultBillingPolicy 在结算层按「空结果不计费」白名单拦截扣费。
// 命中条件（用户在名单内且满足任一）：
...
func applyEmptyResultBillingPolicy(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, quota *int, extraContent *[]string) bool {
	...
	*quota = 0
	if extraContent != nil {
		if localCount {
			*extraContent = append(*extraContent, "该用户已配置空结果不计费，无上游 usage，未扣费")
		} else {
			*extraContent = append(*extraContent, "该用户已配置空结果不计费，上游输出为 0，未扣费")
		}
	}
	return true
}

// markEmptyResultBillingSkipped 在消耗日志 other.admin_info 写入白名单 skip 审计字段。
func markEmptyResultBillingSkipped(other map[string]interface{}) {
	...
	adminInfo["billing_skip_reason"] = "skip_bill_on_empty_result_user"
}
