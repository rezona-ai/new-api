...
> 状态：方案确认（已过两轮 codex 只读核验并按结论修订），待实现
> 日期：2026-08-06
...
| 5 | **Gemini 原生格式**（`/v1beta/...:generateContent`） | 路由 `router/relay-router.go:189`；`relay/channel/gemini/relay-gemini-native.go:20`（非流式）、`:81`（流式）**原样透传** | 原生 `inlineData` base64 | **明确排除**，见 3.2 |
| 6 | `/v1/images/variations` | `router/relay-router.go:154` → `RelayNotImplemented` | — | 不适用 |
| 7 | `/v1/edits` | 路由声明为 image 组（`router/relay-router.go:109`），但 relay mode 是遗留的 `RelayModeEdits`（`relay/constant/relay_mode.go:73`），而 `relayHandler` 只把 `RelayModeImagesGenerations`/`RelayModeImagesEdits` 分发给 `ImageHelper`（`controller/relay.go:38`） | — | **不是可工作的生图入口**，本期不动、不修路由 |
| 8 | MJ notify 回调 `RelayMidjourneyNotify`（`relay/mjproxy_handler.go:91` 直接 `Update()` 裸 `ImageUrl`） | 路由已注释（`router/relay-router.go:217`） | — | 非活动路径；**恢复该路由时必须接入转存**（写进代码注释） |
...
| Replicate | url（仅显式 `b64_json` 才转 base64） | `replicate/adaptor.go:281` |
| Zhipu 4V | **优先用上游 base64**（`b64_json`/`b64_image`），缺失时才由网关下载 URL 转 base64；输出结构只有 `b64_json` | `zhipu_4v/image.go:93-124` |
| Gemini（含 Vertex Imagen，最终委托 Gemini handler `vertex/adaptor.go:330`） | **恒定 b64_json**（上游只给 `BytesBase64Encoded`） | `relay/channel/gemini/relay-gemini.go:1655` |
...
- `service/gcs_storage.go:59` GCS client 初始化只看视频开关；视频 worker 又用 `GCSStorageReady()` 作为启动条件（`gcs_transfer.go:155,228`）——不能简单改成"两开关取或"。见 4.5
- `service/gcs_transfer.go:49-78` `gcsObjectContentType`：白名单与扩展名映射面向视频资产（mp4/jpg），缺 webp/gif。图片单独实现
- `service/download.go:52` `DoDownloadRequest`：非 Worker 分支用 `http.Client.Get`、Worker 分支用 `http.Client.Post`（`:23`），**两条都无 context**。需新增 context 版本（两条分支都要绑）
...
type TransferOpts struct {
    ...
    ReuseExisting bool          // 固定对象名入口（MJ / Responses）：已存在视为幂等命中
...
- **取流**：新增 `service.DoDownloadRequestWithContext(ctx, url, reason...)`——**Worker 分支与非 Worker 分支都要绑 context**（现状 `download.go:23` 的 Worker POST 与 `:68` 的 Get 都无 context）。SSRF 校验沿用 `ValidateURLWithFetchSetting`。原 `DoDownloadRequest` 保留为 `context.Background()` 包装，既有调用方零改动。
...
- **上传**：`GCSUploadObject`（条件写，错误路径不 finalize）。**`ReuseExisting` 语义**：
  ...
  - `true`（MJ / Responses：固定对象名，可能多实例并发或事件重复）：`ErrGCSObjectExists` 视为幂等命中，但**必须走完整性校验**——本次已完整读流时用 `GCSVerifyExistingObject(objectName, 字节数, CRC32C)`；未读完时退化为 `attrs.Size > 0`。**不允许只检查 Size>0 就复用**（半截对象会被永久复用，`gcs_storage.go:167` 的既有契约）
- **需要 base64 时**：`io.TeeReader` 同时喂 GCS 与内存缓冲（受同一体积上限约束）。上游本就是 base64 时直接复用已解码字节。
- **签名**：`GCSSignObjectURL`（4.4），TTL = `GCS_IMAGE_SIGNED_URL_TTL`（默认 7 天）。
...
现状有两个图片渠道在正文写入失败后返回 relay error：Jimeng（`jimeng/image.go:82`）、MiniMax（`minimax/image.go:206`）。改造后 buffered 模式的 `Write` 只写内存、**不会**暴露真实客户端 I/O 错误，这两条分支实际不再触发；真正的客户端写失败推迟到 `ImageHelper` 内的最终提交点。
...
5. **`metadata` 处理只针对 Ali**：Ali 把完整上游响应写进顶层 `metadata`（`ali/image.go:268,279`），内含上游直链/base64；但 **MiniMax 的顶层 `metadata` 是上游业务字段**（`minimax/image.go:34,166-172`），无条件删除会与"未知字段无损保留"直接矛盾。因此开关命名为 `GCS_IMAGE_DROP_ALI_METADATA`（默认 `true`），**仅当渠道/API 类型为 Ali 时生效**，其他渠道的 `metadata` 一律保留
6. **实体 header 清理**：改写后 body 与上游 header 不再一致。`IOCopyBytesGracefully` 只跳过 `Content-Length`（`service/http.go:103-113`、白名单 `service/http.go:45`），因此改写路径必须额外删除 `ETag`、`Content-MD5`、`Digest`、`Content-Digest`、`Repr-Digest`、`Content-Encoding`、`Content-Range`、`Last-Modified`，并按新 body 重设 `Content-Length`
...
type SignPolicy struct {
    ...
    CacheTag        string        // 缓存分区标签（"video" / "image" / "mj"）
}

// GCSSignObjectURL 现签 V4 GET URL：按 policy 决定 TTL 与保留期收口。
// 缓存 key = objectName|CacheTag|TTL|RetentionAnchor；命中时额外校验
// 缓存的 expiresAt 未超过当前 policy 允许的上限，否则丢弃重签。
...
现状 `GCSStorageReady()` 同时表达「client 可用」和「视频转存开启」，被视频 worker 当作启动条件（`gcs_transfer.go:155,228`）。若把 client 初始化改成"两开关取或"，只开图片时会意外启动视频 worker。另外 MJ 一旦存了 `gs://`，之后关闭写开关重启就再也读不出历史结果。
...
**运维须知**：两个写开关都关掉但仍需读历史 `gs://` 结果时，必须显式 `GCS_READ_ONLY_ENABLED=true`。已核验 `GCSStorageReady()` 的现有出现仅三处（定义 `gcs_storage.go:93`、`gcs_transfer.go:155`、`:228`），改动时逐一审计。
...
`relay/helper/stream_scanner.go:205-209` 在**整个 dataHandler 回调期间持有 `writeMutex`**；而 ping goroutine 争同一把锁且只等 10 秒（`:155-170`），超时即判 `StreamEndReasonPingFail` 直接结束流。因此在回调里做「下载 + 上传 + 签名」是高风险动作：给 10s 预算恰好等于 ping 超时，任何额外开销都会打断流。
...
| Responses **非流式**（`relay_responses.go:20-44`） | 在 `IOCopyBytesGracefully`（`:43`）之前对**原始 JSON** 做 patch：遍历 `output[]`，`type=="image_generation_call"` 且 `result` 非空 → 转存后注入 `result_url`，**`result` 的 base64 原样保留**（官方 SDK 按 base64 解析，改掉即破坏 schema） |
...
规定：对象名带请求级命名空间 `{GCS_IMAGE_PREFIX}/resp/{requestID}/{seq}.{ext}`，`seq` 取 `output` 数组下标（无 ID 时也稳定）。**去重只在请求内做**：维护 `itemID(或 seq) → ImageTransferResult` 的请求级 map，同一图片只上传一次。`ReuseExisting=true` 仅用于同请求内的重复事件，且按 4.1 做完整性校验。
...
2. **有界并发 worker，首版就要做**：MJ 只有一个 master 无限循环（`controller/midjourney.go:23`、`main.go:133`），按渠道、任务串行。默认每图 60s 会造成 `N×60s` 最坏阻塞，拖慢所有状态更新与失败退款。首版即用有界工作队列（并发上限 `GCS_IMAGE_CONCURRENCY`，单图预算 `GCS_IMAGE_MJ_TIMEOUT` 默认 `30s`），不能"先串行、积压再异步化"。
3. **幂等**：多实例可能在 CAS 前重复上传 → 固定对象名 `{GCS_IMAGE_PREFIX}/mj/{mjId}.{ext}` + `ReuseExisting=true` + 完整性校验（4.1）。
...
| 出口 | 位置 | 改造 |
|------|------|------|
| `/mj/image/{mjId}` 代理 | `relay/mjproxy_handler.go:29-60` | `gs://` → **302 到现签 URL**；超保留期 → 410；签名失败 → 503。**分支必须紧跟 `GetByOnlyMJId`（`:31`）之后**，先于渠道查询与代理客户端初始化（`:38-49`，无效代理会直接 400）与 SSRF 校验（`:53`）——历史 `gs://` 对象既不依赖渠道也不依赖代理。视频入口已是这个顺序（`controller/video_proxy.go:61`） |
...
MJ 的 `FinishTime` 是**毫秒**（`controller/midjourney.go:140` 抄上游值、`relay/mjproxy_handler.go:611` 为 `UnixNano()/int64(time.Millisecond)`），而 `GCSSignObjectURL` 的保留期锚点是 **Unix 秒**（`gcs_storage.go:197-221`）。直接传入会让保留期截止落到几万年后，过期检查形同失效、必然签出死链，且不会有任何报错。

规定：签名前统一 `/1000`；**且当上游 `FinishTime == 0` 时用当前毫秒补齐**（否则 `/1000` 仍为 0，等于关闭保留期检查）。两条都要有单测钉死。
...
| `GCS_IMAGE_SIGNED_URL_TTL` | `168h`（7 天） | V4 上限；无记录不能重签 |
...
真实顺序：定价 `ModelPriceHelper`（`controller/relay.go:152`）→ **预扣 `PreConsumeBilling`（`controller/relay.go:164`）** → 渠道重试循环（`:190`）→ `DoResponse`（`relay/image_handler.go:114`）→ `n` 修正（`:121-134`）→ `PostTextConsumeQuota`（`:160`）。退款只发生在**所有重试都失败后**的 defer 里（`controller/relay.go:169-175`）。quota 用 `PriceData.OtherRatios`（`service/text_quota.go:304`）。
...
| 15 | — | 代码注释：MJ notify 路由（`mjproxy_handler.go:91`）恢复时必须接入转存 | `relay/mjproxy_handler.go` |
| 16 | — | 用户文档 + CHANGELOG：7 天 TTL 契约、`response_format` 适配表、MJ 链接语义、Gemini 原生排除、`/v1/edits` 非生图入口、错误路径语义变化、耗时口径变化 | 文档 |
...
4. **内存驻留高于直觉**：OpenAI 与 Gemini 本来就先 `ReadAll` 整个响应（`relay-openai.go:555`、`relay-gemini.go:1624`），再叠加缓冲 writer 副本、raw JSON map、解码后图片字节、TeeReader 缓冲与 marshal 结果，峰值可达图片大小的 **3-5 倍**。由 `GCS_IMAGE_MAX_SIZE`(32 MiB) + `GCS_IMAGE_CAPTURE_MAX`(64 MiB) 双重约束，**上线前必须做大图并发压测**。
...
12. **已知残余泄露点**：MJ `code=21` 提交回执（`mjproxy_handler.go:647`）与关闭 `GCS_IMAGE_DROP_ALI_METADATA` 时的 Ali `metadata`，仍可能含上游直链。均为非结果出口，本期接受。
...
2. 写客户端失败的语义未定义（buffered 下 `Write` 不暴露 I/O 错误，错误延迟到最终提交；Jimeng `jimeng/image.go:82`、MiniMax `minimax/image.go:206` 是现存"写后返错"分支）→ 新增决策 8 与 4.2.3
3. **`GCS_IMAGE_DROP_METADATA` 会误删 MiniMax 的业务 metadata**（`minimax/image.go:34,166-172`）→ 改为 `GCS_IMAGE_DROP_ALI_METADATA`，仅 Ali 生效（4.3）
