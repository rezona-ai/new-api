...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string) (stat Stat, err error) {
	...
	// 入口 RPM：最近 60 秒消费 + 错误日志条数（网关写了请求日志的入口量）
	entryRpmQuery := LOG_DB.Table("logs").Select("count(*) entry_rpm")
	...
	if entryRpmQuery, err = applyExplicitLogTextFilter(entryRpmQuery, "username", username); err != nil {
		return stat, err
	}
	if tokenName != "" {
		tx = tx.Where("token_name = ?", tokenName)
		rpmTpmQuery = rpmTpmQuery.Where("token_name = ?", tokenName)
		entryRpmQuery = entryRpmQuery.Where("token_name = ?", tokenName)
	}
	if startTimestamp != 0 {
	...
	if entryRpmQuery, err = applyExplicitLogTextFilter(entryRpmQuery, "model_name", modelName); err != nil {
		return stat, err
	}
	if channel != 0 {
		tx = tx.Where("channel_id = ?", channel)
		rpmTpmQuery = rpmTpmQuery.Where("channel_id = ?", channel)
		entryRpmQuery = entryRpmQuery.Where("channel_id = ?", channel)
	}
	if group != "" {
		tx = tx.Where(logGroupCol+" = ?", group)
		rpmTpmQuery = rpmTpmQuery.Where(logGroupCol+" = ?", group)
		entryRpmQuery = entryRpmQuery.Where(logGroupCol+" = ?", group)
	}

	tx = tx.Where("type = ?", LogTypeConsume)
	rpmTpmQuery = rpmTpmQuery.Where("type = ?", LogTypeConsume)
	entryRpmQuery = entryRpmQuery.Where("type IN ?", []int{LogTypeConsume, LogTypeError})

	// 只统计最近60秒的rpm/entry_rpm/tpm
	rpmWindowStart := time.Now().Add(-60 * time.Second).Unix()
	rpmTpmQuery = rpmTpmQuery.Where("created_at >= ?", rpmWindowStart)
	entryRpmQuery = entryRpmQuery.Where("created_at >= ?", rpmWindowStart)
	...
	if err := entryRpmQuery.Scan(&stat).Error; err != nil {
		common.SysError("failed to query entry rpm stat: " + err.Error())
		return stat, errors.New("查询统计数据失败")
	}
